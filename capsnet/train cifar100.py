...
train_dataset = datasets.CIFAR100( root='./', train=True, transform=tf, download=True)
train_loader = data.DataLoader(train_dataset, batch_size=8, shuffle=True)

net = nets.CapsuleNet([3,32,32],100,3).to(device)
...
#        print(l.size())
        l=l.to(device)
        label=torch.zeros(label.size(0),100).scatter_(1,label.cpu().reshape(-1,1),1).to(device)
        x,z=net(l)
